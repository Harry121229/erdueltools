...
impl PlayerGameData {
    ...
    pub fn character_name(&self) -> String {
        ...
        String::from_utf16_lossy(&self.character_name[..len])
    }
}
...
impl EquipGameData {
    /// EquipGear 为每个装备槽记录的库存绝对索引。
    /// 这里只读，用于在相同物品 ID 的多个实例中识别当前装备的是哪一个。
    pub fn inventory_index(&self, slot: usize) -> Option<u32> {
        self.unk8.get(slot).copied()
    }
}
